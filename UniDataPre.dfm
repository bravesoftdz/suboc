object dmPre: TdmPre
  ...
  object unqrySeries: TUniQuery
    ...
    SQL.Strings = (
      ...
      'WHERE TIPODOC_CONTADOR='#39'PR'#39
  ...
  object unstrdprcGetRecibos: TUniStoredProc
    StoredProcName = 'PRC_CREAR_RECIBOS_PRESUPUESTO'
    SQL.Strings = (
      
        'CALL PRC_CREAR_RECIBOS_PRESUPUESTO(:pSERIE_FACTURA, :pNRO_FACTUR' +
        'A)')
    Connection = dmConn.conUni
    Left = 888
    Top = 288
    ...
    CommandStoredProcName = 'PRC_CREAR_RECIBOS_PRESUPUESTO'
  end
  object unstrdprcCalcularFactura: TUniStoredProc
    StoredProcName = 'PRC_CALCULAR_PRESUPUESTO'
    SQL.Strings = (
      'CALL PRC_CALCULAR_PRESUPUESTO(:pidseriefactura, :pidnumfactura)')
    Connection = dmConn.conUni
    Left = 64
    Top = 288
    ParamData = <
    ...
    CommandStoredProcName = 'PRC_CALCULAR_PRESUPUESTO'
  end
  object unqryDibujos: TUniQuery
    SQLInsert.Strings = (
      'INSERT INTO suboc_dibujos_presupuestos'
      '  (NRO_FACTURA, SERIE_FACTURA, DIBUJO_FACTURA)'
      'VALUES'
      '  (:NRO_FACTURA, :SERIE_FACTURA, :DIBUJO_FACTURA)')
    SQLDelete.Strings = (
      'DELETE FROM suboc_dibujos_presupuestos'
      'WHERE'
      
        '  NRO_FACTURA = :Old_NRO_FACTURA AND SERIE_FACTURA = :Old_SERIE_' +
        'FACTURA')
    SQLUpdate.Strings = (
      'UPDATE suboc_dibujos_presupuestos'
      'SET'
      
        '  NRO_FACTURA = :NRO_FACTURA, SERIE_FACTURA = :SERIE_FACTURA, DI' +
        'BUJO_FACTURA = :DIBUJO_FACTURA'
      'WHERE'
      
        '  NRO_FACTURA = :Old_NRO_FACTURA AND SERIE_FACTURA = :Old_SERIE_' +
        'FACTURA')
    SQLLock.Strings = (
      'SELECT * FROM suboc_dibujos_presupuestos'
      'WHERE'
    ...
    SQLRefresh.Strings = (
        ...
        'SELECT NRO_FACTURA, SERIE_FACTURA, DIBUJO_FACTURA FROM suboc_dib' +
        'ujos_presupuestos'
      'WHERE'
      '  NRO_FACTURA = :NRO_FACTURA AND SERIE_FACTURA = :SERIE_FACTURA')
    SQLRecCount.Strings = (
      'SELECT COUNT(*) FROM suboc_dibujos_presupuestos')
    Connection = dmConn.conUni
    SQL.Strings = (
      'SELECT * FROM suboc_dibujos_presupuestos'
      'WHERE NRO_FACTURA = :NRO_FACTURA'
  ...
  object unstrdprcCrearFactura: TUniStoredProc
    StoredProcName = 'PRC_CREAR_PRESUPUESTO_FACTURA'
    SQL.Strings = (
      
        'CALL PRC_CREAR_PRESUPUESTO_FACTURA(:pidseriefactura, :pidnumfact' +
        'ura, :pidseriefacturaabono, :pfechafacturaabono, @pidnumfacturaa' +
        'bono); SELECT @pidnumfacturaabono AS '#39'@pidnumfacturaabono'#39)
    Connection = dmConn.conUni
    Left = 1016
    Top = 288
    ParamData = <
    ...
    CommandStoredProcName = 'PRC_CREAR_PRESUPUESTO_FACTURA'
  end
end
